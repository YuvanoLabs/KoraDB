...
- **Secured server mode couples authentication to TLS.** The authenticated server configuration
  cannot run as plaintext. The CLI does not yet refuse to attach a supplied token to a plaintext
  connection, so never supply `--token`/`KoraDB_TOKEN` when connecting to an `--insecure` server.
...
- Tokens look like `pdb_<keyID>_<secret>` — 64 bits of key id + 256 bits of secret. The `pdb_`
...
- The **first** admin key is created with the server **stopped**:
  `KoraDB-server bootstrap --db data.db --role admin`.
...
- Safe last-admin protection and a tested break-glass recovery workflow.
...
- `--insecure` disables both TLS and authentication. It currently defaults to `:50051`, which can
  listen on non-loopback interfaces. Always pass an explicit loopback address such as
  `--addr 127.0.0.1:50051`; firewall it as well.
- The CLI can attach a token to a plaintext remote connection when no TLS option is configured.
  Never reuse or supply a real token in insecure development mode. A release should fail closed
  on this combination unless a separate explicit override is provided.
- The first bootstrap command accepts any role even though operational recovery needs an admin
  principal. Bootstrap an admin key and protect it.
- It is possible to revoke the last admin key. Recovery then requires stopping the server and
  bootstrapping a new admin key against the exclusively locked file.
...
- The API has no application-level pagination or query-complexity controls. Do not expose it to
  untrusted tenants.
